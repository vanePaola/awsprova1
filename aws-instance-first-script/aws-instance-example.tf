resource "null_resource" "terraform-debug" {
  ...
  provisioner "local-exec" {
    command = "echo $VARIABLE1 > debug.txt ; cat $VARIABLE1 > debug2.txt ;echo [webservers] > /var/lib/jenkins/workspace/ansible-tomcat/dev.inv ; echo $VARIABLE2 ansible_user=ec2-user >> ../../../ansible-tomcat/ansible-tomcat/dev.inv ;  "
